...
# Test the scanner with real KLSE tickers
from buffett.scanner import run_weekly_scan

print('Testing scanner with real KLSE tickers...')
...
try:
    ...
    # Clear existing universe and add real test tickers (using known good ones)
    cursor.execute("DELETE FROM buffett_universe")
    # Using tickers we know work: 1155.KL (Maybank), 1295.KL (Public Bank), 5347.KL (Tenaga)
    # But we need to map them to our ticker format
    test_tickers = [
    ...
    # Now test the scanner with real data
